...
static void draw_callback(Canvas* canvas, void* ctx) {
    ...
    canvas_draw_str(canvas, 10, 35, "Use arrow keys");
    canvas_draw_str(canvas, 10, 45, "Press BACK to exit");

    char pos[32];
    snprintf(pos, sizeof(pos), "Line:%u Col:%u", state->cursor_line, state->cursor_col);
    canvas_draw_str(canvas, 10, 55, pos);
}
